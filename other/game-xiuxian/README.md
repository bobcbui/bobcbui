...
│   │   ├── equipment.js   # 装备：掉落生成、穿戴/卸下、属性加成
...
- 顶部显示**游戏总等级**、总经验条、永久属性加成（每总等级 +3 攻 / +10 生命）、最高通关关卡
- 底部四页菜单：
  1. **关卡**：上一关/下一关选择关卡（受解锁进度限制），进入所选关卡
  2. **装备**：6 部位穿戴/卸下、背包管理（关卡结算掉落）
  3. **技能**：技能卡与强化卡图鉴
  4. **活动**：成就列表与活动占位
- 通关解锁下一关，战败可重试当前关；每局结算总经验：通关 `100 + 关卡×40`；战败 `击杀×2 + 波次×15`

### 装备系统（主页·装备页）

- 6 部位（武器/头盔/衣服/鞋子/戒指/项链）× 6 品质（凡品→仙品）
- **获取**：每关结算掉落 —— 通关必得 1 件（品质随关卡提升），战败 30% 概率
- 永久属性加成（攻击/防御/生命/速度），跨关保留并写入存档
