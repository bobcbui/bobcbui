...
export function tryUseSkill(skillId){
  ...
  if(def.type==='basic'){ const s=window.setStatus; if(s)s('飞剑术会自动作为普通攻击释放',1.2); return; }
  ...
  const skillLv = P.skillLevels?.[def.id] || 1;
  const dmg = Math.round((P.atk + (P.level*0.5)) * def.baseDmg * (1 + (skillLv - 1) * 0.18));
  ...
  switch(def.type){
    ...
    case 'aoe':
      ...
      break;
    case 'water':
      if(scene.doWaterDomain) scene.doWaterDomain(tx, ty, dmg, def.aoeRadius||150, def.color||0x5aa6b1);
      break;
    case 'tornado':
      if(scene.doTornadoSkill) scene.doTornadoSkill(tx, ty, dmg, def.aoeRadius||165, def.color||0x9fb884);
      break;
    case 'multi':
