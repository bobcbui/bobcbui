...
Java环境配置就是运行java文件和编辑Java文件的一些软件和系统设置，Java环境又区分编辑Java文件的环境（开发环境）和运行Java文件的环境（生产环境）。

编辑Java的环境就是一些文本编辑器，例如VSCODE, IDEA，记事本， 这些软件可以很好的编辑Java文件。

运行Java的环境就需要JDK(Java Development Kitx)即Java开发工具包， 阿里，腾讯, 华为， Oracle, 微软都开发了JDK, 我们可以使用自己喜欢的公司开发的JDK。
